...
resolution:
  ...
  resolved_by: "Claude Code, per prompt-6a3b7c52 (commit 3ddefc3)"
  fix_description: >
    (a) main.py: debug handler capped at 10 MB, ten backups, dead
    mode='w' dropped, rolls over once at startup when non-empty. (b)
    PerformanceMonitor construction moved after _load_config and now
    reads config.fps_limit (confirmed live: manager.py:166,
    PerformanceMonitor(target_fps=_fps), no longer the literal 60).

verification:
  verified_date: "2026-08-07"
  verified_by: "Claude Code (source re-check); William Watson (gtach.local)"
  test_results: >
    Report: rotation test discriminates against HEAD, +59/-9 lines, one
    commit. Source re-check confirms both fixes live. William confirmed
    2026-08-07 that GTach functions correctly on gtach.local.
  closure_notes: >
    Closed on William's confirmation. Explicitly not a fix for the
    ai/task.md §9.12 sluggishness, whose cause remains undistinguished
...
version_history:
  ...
  - version: "1.1"
    ...
    author: "William Watson"
    changes:
      - "Status open -> closed. change-6a3b7c52 implemented (commit 3ddefc3), confirmed by source re-check: both the rotation fix and the PerformanceMonitor ordering fix are live."
