...
resolution:
  ...
  resolved_by: "Claude Code, per prompt-2b6f4d91"
  fix_description: >
    Palette toggle wired into TouchHandler._handle_long_press (the live
    path), gated to RADIAL, calling DisplayManager._toggle_palette
    directly rather than through the inert coordinator registration.
    The inert DOUBLE_TAP registration removed from manager.py. The
    inert SWIPE_DOWN/SWIPE_UP registrations at manager.py:183-188 left
    in place with an explanatory comment, as this triple's own scope
    excluded repairing the coordinator's dead dispatch mechanism.
...
verification:
  ...
  test_results: >
    ...
    _handle_long_press gated to RADIAL, and logging 'Palette switched to
    {name}'; no DOUBLE_TAP reference remains in manager.py.
  closure_notes: >
    William confirmed on 2026-08-07 that GTach is functioning correctly
    on gtach.local, which this triple's own deliverable (the night
    palette becoming reachable) is part of. GOVERNANCE GAP, NOT A
    SOURCE DEFECT: this triple does not appear anywhere in ai/task.md,
    despite being implemented, and was discovered only by an unrelated
    source re-check while closing issue-5012004e. Its own §6.1/§9.8.5
    item 3 contrast question (night palette blue at 1.55:1) remains an
    open design decision, unaffected by this closure. The coordinator's
    dead gesture-callback mechanism and the now-inert SWIPE registrations
    at manager.py:183-188 remain as documented, deliberate residuals,
    not defects of this triple.
...
version_history:
  ...
  - version: "1.1"
    ...
    author: "William Watson"
    changes:
      - "Status open -> closed. change-2b6f4d91 confirmed implemented by source re-check: _toggle_palette present and wired to the live long-press path, no DOUBLE_TAP reference remains."
      - "Recorded that this triple was entirely absent from ai/task.md despite being implemented — a governance-tracking gap, discovered while closing issue-5012004e, corrected by this closure."
...
| 1.1 | 2026-08-07 | Status open → closed. Resolution and verification recorded by source re-check; recorded as untracked in ai/task.md, a governance gap now corrected. Closed on William's confirmation that GTach functions correctly on gtach.local. |
