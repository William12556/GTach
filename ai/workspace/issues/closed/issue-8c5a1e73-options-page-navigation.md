...
resolution:
  ...
  resolved_by: "Claude Code, per prompt-8c5a1e73 (commit 31e8524)"
  fix_description: >
    Options menu paged: page 0 holds Simulation/Bluetooth and Debug,
    page 1 holds Clear settings and Check for updates, wrapping in
    both directions via horizontal swipe from TouchHandler. The view
    key carries the page index. Confirmed live in source: manager.py's
    _register_options_menu_regions branches on self._options_page,
    with page 1's specs including ('clear_settings',
    TouchAction.SETTINGS_CHANGE, lambda pos: self._on_clear_settings_requested()).

verification:
  verified_date: "2026-08-07"
  verified_by: "Claude Code (source re-check); William Watson (gtach.local)"
  test_results: >
    Report: EDIT 2 (the view key) proved first and independently —
    _current_view_key returns distinct tuples for pages 0 and 1. Forty
    checks: 14/40 pre-change, 40/40 post-change. Source re-check
    confirms the page-branching registration and the Clear settings
    binding are live. William confirmed 2026-08-07 that GTach
...
version_history:
  ...
  - version: "1.1"
    ...
    author: "William Watson"
    changes:
      - "Status open -> closed. change-8c5a1e73 implemented (commit 31e8524), confirmed by source re-check: page-branching registration and Clear settings binding both live."
